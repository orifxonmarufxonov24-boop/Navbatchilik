...
# --- TABLAR ---
tab1, tab2, tab3, tab4, tab5 = st.tabs(["📝 Navbatchilik", "🛠️ Naryad", "📊 Statistika", "📸 Yo'qlama", "📨 Xabarlar"])
...
# ============================================================================
# YO'QLAMA BO'LIMI - Face ID Tizimi
# ============================================================================
with tab4:
    st.subheader("📸 Yo'qlama - Face ID Tizimi")
    ...
    with yoqlama_tab1:
        ...
        st.info("📌 Kameradan rasm oling yoki fayl yuklang. Tizim yuzlarni aniqlab, kim kelganini ko'rsatadi.")
        ...
        if input_method == "📷 Kamera":
            camera_image = st.camera_input(
                "Rasmga oling",
                key="yoqlama_camera",
                help="Yuzlar aniq ko'rinishi kerak"
            )
            if camera_image:
            ...
            if uploaded_file:
                frame_image = uploaded_file.read()
    ...
    with yoqlama_tab3:
        ...
        st.info("📌 Talabani tanlang va yuzini kameradan oling yoki rasm yuklang.")
        
        # Talabalar ro'yxatidan tanlash
        student_options_for_face = df.apply(
        ...
        selected_student = st.selectbox(
            ...
            key="face_student_select"
        )
        
        # Rasm olish
        face_input_method = st.radio(
            "Rasm olish usuli",
            ["📷 Kameradan rasm olish", "📁 Fayldan yuklash"],
            horizontal=True,
        ...
        if face_input_method == "📷 Kameradan rasm olish":
            camera_face = st.camera_input(
                "Yuzni rasmga oling",
                key="face_camera",
                help="Talaba yuzini to'g'ridan-to'g'ri kameraga qaratsin"
            )
            if camera_face:
        ...
        if face_image:
            try:
                ...
                try:
                    ...
                    st.warning("⚠️ Yuz aniqlash moduli yuklanmadi.")
                    face_detected = True
        ...
        if len(filtered_students) > 0:
            ...
            if selected_for_edit:
                ...
                st.markdown("**📷 Yangi yuz rasmi (ixtiyoriy):**")
                new_face_image = st.file_uploader(
                    "Yangi rasm yuklang",
                    type=["jpg", "jpeg", "png"],
                    key=f"edit_face_{selected_idx}"
                )
                
                if st.button("💾 O'zgarishlarni Saqlash", type="primary", key=f"save_edit_{selected_idx}"):
                    try:
                        sheet = get_main_sheet()
                        row_num = original_df_idx + 2
                        
                        sheet.update_cell(row_num, 1, new_name)
                        sheet.update_cell(row_num, 2, new_room)
                        if 'telefon raqami' in original_row:
                            sheet.update_cell(row_num, 3, new_phone)
                        
                        if new_face_image:
                            import face_module as fm
                            image_base64 = base64.b64encode(new_face_image.read()).decode()
                            fm.register_student(current_student_id, new_name, image_base64)
                        
                        st.success("✅ O'zgarishlar saqlandi!")
                        log_activity("Talaba tahrirlandi", f"{original_row['ism familiya']} -> {new_name}")
                        st.rerun()
